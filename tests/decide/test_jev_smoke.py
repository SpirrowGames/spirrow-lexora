...
One request, one ``noul`` question, one billed call. The ``score`` path
(:func:`test_real_score_roundtrip`) is a SECOND billed call and is also
skipped unless ``JEV_SMOKE_WITH_SCORE=1`` is set (msg-368 v9):

    TYPESAFE_API_KEY=... JEV_SMOKE_WITH_SCORE=1 \
        uv run --extra dev pytest -m smoke tests/decide/test_jev_smoke.py -rs

The point is to confirm the wire format in :mod:`lexora.decide.jev_client`
(taken from Fermi's quote of docs.typesafe.ai/api.md, T-decide-jev-provider msg-336)
...
async def test_real_score_roundtrip() -> None:
    """Score path against the live API: legend dict → list normalisation
    (msg-368 v9). Second billed call; opt-in via JEV_SMOKE_WITH_SCORE=1."""
    if os.environ.get("JEV_SMOKE_WITH_SCORE") != "1":
        pytest.skip("JEV_SMOKE_WITH_SCORE=1 not set (second billed call)")
    api_key = os.environ.get("TYPESAFE_API_KEY")
    if not api_key:
        pytest.skip("TYPESAFE_API_KEY not set")
    provider = JevProvider(api_key, timeout_ms=30_000)
    result = await provider.evaluate(
        state="The customer wrote: 'It arrived late, but it works fine.'",
        questions={
            "satisfaction": QuestionSpec(
                type="score",
    ...
    answer = result.answers["satisfaction"]
    legend = answer["legend"]
    assert isinstance(legend, list)
    assert legend and all(isinstance(x, str) for x in legend)
    assert 0.0 <= answer["score"] <= len(legend) - 1
