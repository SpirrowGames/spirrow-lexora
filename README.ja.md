...
- 未登録のモデル名は`default_backend`にルーティング**しない** — `404` / `code: model_not_found` で拒否する。複数の backend が同じモデル名を宣言している場合も、選びようがないので同じく拒否する（ティア名で指定する。拒否メッセージには、競合している backend に届くティア名だけが列挙される）。どちらも `WARNING`（`model_unknown_refused` / `model_ambiguous_refused`）に出る ∴ 出荷ログレベル（INFO）でも観測できる。マルチバックエンド時（`routing.enabled: true`）の挙動で、レガシー単一バックエンドモードは選択肢が無いため従来どおり任意の名前を受ける
- `/v1/models`は全バックエンドのモデルを集約して返却。ただしマルチバックエンド時は**ルーティング可能な名前だけ**に絞る（複数 backend が宣言している名前、および upstream だけが知っている名前 — vLLM は旧名を別名で serve し続ける — は載せない）。載せてから 404 で断るのは「存在しない口を広告する」ことになるため
