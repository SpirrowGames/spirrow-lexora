...
- 未登録のモデル名は`default_backend`にルーティング**しない** — `404` / `code: model_not_found` で拒否する。複数の backend が同じモデル名を宣言している場合も、選びようがないので同じく拒否する（ティア名で指定する。拒否メッセージには、競合している backend に届き**かつ要求されたモデル名に解決する**ティア名だけが列挙される。届くが別のモデルを serve するティアは挙げない — 従うと要求していないモデルが答えることになり、この拒否が防いでいるすり替えを gateway 自身が指示する形になるため）。どちらも `WARNING`（`model_unknown_refused` / `model_ambiguous_refused`）に出る ∴ 出荷ログレベル（INFO）でも観測できる。マルチバックエンド時（`routing.enabled: true`）の挙動で、レガシー単一バックエンドモードは選択肢が無いため従来どおり任意の名前を受ける
- `/v1/models`は全バックエンドのモデルを集約して返却。ただしマルチバックエンド時は**ルーティング可能な名前だけ**に絞る（複数 backend が宣言している名前、および upstream だけが知っている名前 — vLLM は旧名を別名で serve し続ける — は載せない）。載せてから 404 で断るのは「存在しない口を広告する」ことになるため。具体モデル行の `backend` は、その名前を**宣言している** backend であって、upstream がその名前を報告した backend ではない（upstream を共有する backend は互いのカタログを丸ごと報告するため一致しない）∴ router が実際に送る先と一致する。広告される集合は routable な集合と**一致する**（両方向）— routable な名前が隠れることも無い。どの upstream も報告しない宣言済みの名前も、このゲートウェイ自身の権限で広告する（`owned_by: "lexora"` / `created: 0` / `type: "declared"`）。「upstream が報告した」ことは routable であることの根拠ではないし、そもそもカタログを持たない backend では取得できない（`claude_code` の `list_models` は空リストを返す ∴ `claude-code-*` の 2 名は routable なのに `/v1/models` が存在を否定していた。しかもこの backend に向く tier が無いので tier 経由でも辿り着けなかった）。`type` を持たない行は upstream が実際に報告した行で、`created` / `owned_by` は upstream が送ってきた値のまま
