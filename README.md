...
- Unregistered model names are **refused with `404` / `code: model_not_found`**, not routed to `default_backend`. A name declared by two or more backends is refused the same way, because there is no non-arbitrary way to pick between them — use the tier alias instead; the refusal message lists the tiers that reach the colliding backends **and resolve to the name that was asked for**. A tier reaching one of those backends but serving a different model is not offered — following that advice would answer with a model the caller did not request, which is the substitution this refusal exists to prevent, only named by the gateway. Both refusals are logged at `WARNING` (`model_unknown_refused` / `model_ambiguous_refused`), so a caller sending a name this gateway does not serve is visible at the shipping log level instead of being answered by whichever model `default_backend` happens to point at. This applies to multi-backend mode (`routing.enabled: true`); legacy single-backend mode has one backend and no choice to make, so it still accepts any name
- `/v1/models` aggregates models from all backends and appends the configured tier aliases as `{"type": "tier", "id": "<tier>", "resolved_model": "<concrete-id>"}` entries so callers can see both the concrete model IDs the backend serves and the tier names the router accepts. In multi-backend mode the concrete entries are narrowed to the names the router will actually route: a name declared by several backends, or one only the upstream knows about (vLLM keeps pre-rename aliases), is left out rather than advertised and then refused. The `backend` field on a concrete entry names the backend that **declares** the model, which is not always the one whose upstream reported it — backends sharing an upstream each report its whole catalogue — so the field agrees with where the router actually sends the name. The advertised set **is** the routable set, in both directions: a routable name is never hidden either. A declared name that no upstream reports is still advertised, on this gateway's own authority — `owned_by: "lexora"`, `created: 0`, `type: "declared"` — because "some upstream reported it" is not what makes a name routable, and is not even available for a backend that has no catalogue to report (`claude_code` returns an empty list, so both `claude-code-*` names used to route while `/v1/models` denied they existed, with no tier mapping to that backend to reach them by either). A row carrying no `type` is one an upstream did report, with the vendor's own `created` / `owned_by` left as the upstream sent them
